...
The v2 service worker uses cache `simgrid-v2.3.1`. When replacing an older version, open the site once in Safari before reopening the Home Screen app so the new worker can activate.
...
## v2.3.1 — clear Track Academy

- У всех 26 гайдов теперь есть локальная растровая обложка и отдельная схема трассы.
- Для автоматически созданных гайдов добавлены реальные названия ключевых зон и понятные действия по порядку.
- Новый сценарий тренировки: три разминочных круга, одна зона фокуса и серия из пяти чистых кругов.
- Пересобраны карточки и модальное окно для ровной сетки на macOS, iPad и iPhone.
- Убрана перегруженная телеметрическая визуализация из основного разбора зон.
