(() => {
  ...
  function renderGuideCard(g){const t=track(g.track),progress=guideCompletion(g.track),quiz=state.guideQuiz[g.track];return `<button class="guide-card" data-action="guide-detail" data-id="${g.track}" style="text-align:left;color:inherit"><div class="guide-photo"><img class="guide-photo-img" src="${esc(g.photo)}" alt="${esc(t.name)}" loading="lazy" referrerpolicy="no-referrer" onerror="this.style.display='none'"><span class="guide-level">${esc(g.level)}</span><div class="guide-map" style="color:white">${trackSvg(t,'currentColor')}</div></div><div class="guide-content"><div class="guide-title-row"><h3>${esc(t.name)}</h3>${quiz?`<span class="quiz-score">Тест ${quiz.score}/${quiz.total}</span>`:''}</div><p>${esc(g.summary)}</p><div class="guide-progress"><span style="width:${progress}%"></span></div><div class="guide-actions"><span class="corner-count">${g.sectors.length} зон · ${progress}%</span><span class="link-button">Изучить →</span></div></div></button>`;}
