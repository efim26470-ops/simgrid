(() => {
  ...
  function renderGuideCard(g){const t=track(g.track);return `<button class="guide-card" data-action="guide-detail" data-id="${g.track}" style="text-align:left;color:inherit"><div class="guide-photo"><img class="guide-photo-img" src="${esc(g.photo)}" alt="${esc(t.name)}" loading="lazy" referrerpolicy="no-referrer" onerror="this.style.display='none'"><span class="guide-level">${esc(g.level)}</span><div class="guide-map" style="color:white">${trackSvg(t,'currentColor')}</div></div><div class="guide-content"><h3>${esc(t.name)}</h3><p>${esc(g.summary)}</p><div class="guide-actions"><span class="corner-count">${g.sectors.length} ключевых зон</span><span class="link-button">Открыть →</span></div></div></button>`;}
  ...
  function showGuideDetail(trackId){const g=D.guides.find(x=>x.track===trackId);if(!g)return;const t=track(trackId);const body=`<div class="guide-detail-hero"><img src="${esc(g.photo)}" alt="${esc(t.name)}" loading="eager" referrerpolicy="no-referrer" onerror="this.style.display='none'"><div class="guide-detail-title"><span class="pill">${esc(g.level)}</span><h2>${esc(t.name)}</h2><p>${t.length.toFixed(3)} км · ${t.corners} поворотов · ${esc(t.country)}</p></div></div><div class="guide-summary">${esc(g.summary)} Выбери 1–2 сектора и отрабатывай их сериями по 5 чистых кругов.</div><div class="guide-quickfacts" style="margin:16px 0 18px"><div class="metric card"><small>Конфигурации</small><strong>${t.configs.length}</strong><span>${esc(t.configs.slice(0,2).join(' / '))}${t.configs.length>2?'…':''}</span></div><div class="metric card"><small>Ключевые зоны</small><strong>${g.sectors.length}</strong><span>точки для тренировки</span></div><div class="metric card"><small>Сложность</small><strong>${t.difficulty}/5</strong><span>${esc(g.level)}</span></div></div><div class="grid-2"><article><h3>Ключевые зоны</h3><div class="corner-list">${g.sectors.map((s,i)=>`<div class="corner-item"><span class="corner-num">${i+1}</span><div><h4>${esc(s.name)}</h4><p>${esc(s.tip)}</p></div><span class="gear">${esc(s.gear)} передача</span></div>`).join('')}</div></article><div class="stack"><article class="guide-map-panel"><div class="card-head" style="margin-bottom:0"><div><h3>Схема трассы</h3><p>Стартовая точка отмечена акцентом. Используй карту как опорную схему зон.</p></div></div><div class="guide-track-map" style="color:var(--text)">${trackSvg(t,'currentColor')}</div><p class="guide-map-caption">Подсказка: сначала закрепи ориентиры торможения, затем уже атакуй апекс и выход.</p></article><article class="card card-pad"><div class="card-head"><div><h3>Базовый сетап</h3></div></div>${g.setup.map(x=>`<div class="setting-row"><div><h3>${esc(x)}</h3></div><span>✓</span></div>`).join('')}</article><article class="card card-pad"><div class="card-head"><div><h3>Типичные ошибки</h3></div></div>${g.mistakes.map(x=>`<div class="setting-row"><div><h3>${esc(x)}</h3></div><span style="color:var(--danger)">!</span></div>`).join('')}</article></div></div>`;openModal(modalShell(`Гайд · ${t.name}`,body,`<button class="secondary" data-action="new-session-guide" data-id="${t.id}">Записать тренировку</button><button class="primary" data-action="close-modal">Закрыть</button>`));}
