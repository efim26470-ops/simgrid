(() => {
  ...
  function renderGuides(){
    ...
    return `<div class="academy-hero card"><div class="academy-hero-copy"><p class="eyebrow">Track Academy</p><h2>Понятный план тренировки на каждой трассе</h2><p>Выбери трассу, разогрей шины, отработай одну ключевую зону и только затем собирай серию чистых кругов.</p><div class="academy-howto"><span><b>1</b><em>3 круга<br><small>разминка</small></em></span><span><b>2</b><em>1 зона<br><small>фокус</small></em></span><span><b>3</b><em>5 кругов<br><small>серия</small></em></span></div></div><div class="academy-progress" style="--academy-p:${avg}%"><strong>${avg}%</strong><span>общий прогресс</span><small>${completed} гайдов завершено</small></div></div><div class="toolbar guide-toolbar"><div class="toolbar-group"><div class="search"><input id="guideSearch" placeholder="Трасса или название поворота"></div><select class="select compact-select" id="guideLevel"><option value="all">Любая сложность</option><option value="Средний">Средний</option><option value="Продвинутый">Продвинутый</option><option value="Эксперт">Эксперт</option></select></div><span class="pill">${D.guides.length} трасс · фото и схема</span></div><div id="guideGrid" class="guide-grid">${D.guides.map(renderGuideCard).join('')}</div>`;
  }
  function renderGuideCard(g){
    ...
    const hero=hasCover?`<div class="guide-photo"><img class="guide-photo-img" src="${esc(cover)}" alt="Трековая обложка ${esc(t.name)}" loading="lazy" decoding="async" onerror="this.closest('.guide-photo').classList.add('guide-photo--schematic');this.remove()"><span class="guide-level">${esc(g.level)}</span><span class="guide-country">${esc(t.country)}</span><div class="guide-map" aria-hidden="true">${trackSvg(t,'#ffffff')}</div></div>`:`<div class="guide-photo guide-photo--schematic"><span class="guide-level">${esc(g.level)}</span><div class="guide-blueprint"><div class="guide-blueprint-copy"><p class="guide-kicker">Track Academy</p><h4>${esc(t.name)}</h4><div class="guide-blueprint-meta">${esc(t.country)} • ${t.length.toFixed(3)} км • ${t.corners} поворотов</div></div><div class="guide-map guide-map--large" aria-hidden="true">${trackSvg(t,'#ffffff')}</div></div></div>`;
    return `<button type="button" class="guide-card" data-action="guide-detail" data-id="${g.track}" style="text-align:left;color:inherit">${hero}<div class="guide-content"><div class="guide-title-row"><h3>${esc(t.name)}</h3>${quiz?`<span class="quiz-score">Тест ${quiz.score}/${quiz.total}</span>`:''}</div><p>${esc(g.summary)}</p><div class="guide-card-focus"><small>Первая цель</small><strong>${esc(g.primaryGoal||`Отработать ${g.sectors[0]?.name||'ключевую зону'}`)}</strong></div><div class="guide-card-meta"><span>${g.sectors.length} ключевых зон</span><span>${esc(g.sessionPlan||'3 + 5 кругов')}</span></div><div class="guide-progress" aria-label="Прогресс ${progress}%"><span style="width:${progress}%"></span></div><div class="guide-actions"><span class="corner-count">Прогресс ${progress}%</span><span class="link-button">Открыть план →</span></div></div></button>`;
  }
  ...
  function showGuideDetail(trackId){
    ...
    const sectors=g.sectors.map((sector,i)=>`<article class="academy-sector"><div class="sector-head"><span class="corner-num">${i+1}</span><div><p class="sector-label">Ключевая зона ${i+1}</p><h3>${esc(sector.name)}</h3></div><span class="gear">${esc(sector.gear)} передача</span></div><div class="sector-instruction"><span>Что делать</span><p>${esc(sector.tip)}</p></div><div class="sector-facts"><span><small>Торможение</small><b>${esc(sector.braking)}</b></span><span><small>Ориентир входа</small><b>${esc(sector.entrySpeed)}</b></span><span><small>Апекс</small><b>${esc(sector.apex)}</b></span><span><small>Газ</small><b>${esc(sector.throttle)}</b></span></div><p class="sector-curb"><b>Поребрик:</b> ${esc(sector.curb)}</p></article>`).join('');
    ...
    const hero=g.hero?`<div class="guide-detail-hero"><img src="${esc(g.hero)}" alt="Трековая обложка ${esc(t.name)}" loading="eager" onerror="this.closest('.guide-detail-hero').classList.add('guide-detail-hero--schematic');this.remove()"><div class="guide-detail-title"><span class="pill">${esc(g.level)}</span><h2>${esc(t.name)}</h2><p>${t.length.toFixed(3)} км · ${t.corners} поворотов · ${esc(t.country)}</p><div class="guide-configs">${t.configs.slice(0,4).map(x=>`<span>${esc(x)}</span>`).join('')}</div></div><div class="guide-detail-mapfloat">${trackSvg(t,'#ffffff')}</div></div>`:`<div class="guide-detail-hero guide-detail-hero--schematic"><div class="guide-detail-blueprint"><div class="guide-detail-copy"><p class="guide-kicker">Track Academy</p><span class="pill">${esc(g.level)}</span><h2>${esc(t.name)}</h2><p>${t.length.toFixed(3)} км · ${t.corners} поворотов · ${esc(t.country)}</p></div><div class="guide-detail-trackbox">${trackSvg(t,'#ffffff')}</div></div></div>`;
    const setupList=g.setup.map(x=>`<li><span>✓</span>${esc(x)}</li>`).join('');
    const mistakeList=g.mistakes.map(x=>`<li><span>!</span>${esc(x)}</li>`).join('');
    const body=`${hero}<section class="guide-start-panel"><div><p class="eyebrow">Цель тренировки</p><h3>${esc(g.primaryGoal||g.summary)}</h3><p>${esc(g.summary)}</p><small>Скорости и дистанции ниже — стартовые ориентиры. Сначала добейся повторяемой линии, затем подстрой их под машину, топливо и погоду.</small></div><ol class="guide-start-steps"><li><b>1</b><span>Разминка<small>3 спокойных круга</small></span></li><li><b>2</b><span>Одна зона<small>${esc(g.sectors[0]?.name||'первый сектор')}</small></span></li><li><b>3</b><span>Рабочая серия<small>5 чистых кругов</small></span></li><li><b>4</b><span>Проверка<small>средний круг и разброс</small></span></li></ol></section><div class="guide-quickfacts"><div class="metric card"><small>План</small><strong>${esc(g.sessionPlan||'3 + 5')}</strong><span>одна цель за сессию</span></div><div class="metric card"><small>Ключевые зоны</small><strong>${g.sectors.length}</strong><span>разобраны по порядку</span></div><div class="metric card"><small>Прогресс</small><strong>${guideCompletion(trackId)}%</strong><span>${done.filter(Boolean).length} из ${(g.checklist||[]).length} задач</span></div><div class="metric card"><small>Мини-тест</small><strong>${quiz?`${quiz.score}/${quiz.total}`:'—'}</strong><span>${quiz?'последний результат':'ещё не пройден'}</span></div></div><section class="academy-section training-section"><div class="card-head"><div><h2>Выбери режим</h2><p>Меняется приоритет, но порядок работы остаётся тем же</p></div></div><div class="training-modes">${modes}</div></section><div class="academy-layout"><main><section class="academy-section"><div class="card-head"><div><h2>Ключевые зоны по порядку</h2><p>В каждой карточке сначала прочитай действие, затем сверяй ориентиры</p></div></div><div class="academy-sectors">${sectors}</div></section></main><aside class="stack guide-aside"><article class="guide-map-panel"><div class="card-head"><div><h3>Схема трассы</h3><p>${esc(t.configs[0]||'Основная конфигурация')}</p></div></div><div class="guide-track-map" style="color:var(--text)">${trackSvg(t,'currentColor')}</div><p class="guide-map-caption">Иди по зонам сверху вниз. Не пытайся улучшить весь круг за один выезд.</p></article><article class="card card-pad guide-advice"><div class="card-head"><div><h3>Базовая настройка</h3><p>Что проверить перед серией</p></div></div><ul>${setupList}</ul></article><article class="card card-pad guide-advice danger"><div class="card-head"><div><h3>Частые ошибки</h3><p>Что сразу портит круг</p></div></div><ul>${mistakeList}</ul></article><article class="card card-pad"><div class="card-head"><div><h3>Чек-лист тренировки</h3><p>Нажимай после выполнения</p></div></div><div class="guide-checklist">${checklist}</div></article></aside></div>`;
    openModal(modalShell(`Гайд: ${t.name}`,body,`<button class="secondary" data-action="guide-quiz" data-id="${t.id}">${quiz?'Повторить тест':'Мини-тест'}</button><button class="secondary" data-action="new-session-guide" data-id="${t.id}">Записать 5 кругов</button><button class="primary" data-action="close-modal">Закрыть</button>`),'wide');
  }
