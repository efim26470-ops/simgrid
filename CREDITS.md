...
The remaining bundled track photographs were already part of the SimGrid project. Every active Track Academy card now uses a local raster image, so guide covers do not depend on an external image host. Where a track-specific photograph is unavailable, the closest matching bundled circuit environment is used behind that track's own diagram and name. Circuit diagrams, telemetry components, interface artwork and application icons are project assets created for SimGrid.
