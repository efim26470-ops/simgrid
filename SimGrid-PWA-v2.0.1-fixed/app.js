(() => {
  ...
  function renderGuideCard(g){const t=track(g.track),progress=guideCompletion(g.track),quiz=state.guideQuiz[g.track],cover=g.cover||g.photo;return `<button type="button" class="guide-card" data-action="guide-detail" data-id="${g.track}" style="text-align:left;color:inherit"><div class="guide-photo"><img class="guide-photo-img" src="${esc(cover)}" alt="Схема ${esc(t.name)}" loading="lazy" decoding="async" onerror="this.hidden=true"><span class="guide-level">${esc(g.level)}</span><span class="guide-country">${esc(t.country)}</span></div><div class="guide-content"><div class="guide-title-row"><h3>${esc(t.name)}</h3>${quiz?`<span class="quiz-score">Тест ${quiz.score}/${quiz.total}</span>`:''}</div><p>${esc(g.summary)}</p><div class="guide-configs">${t.configs.slice(0,3).map(x=>`<span>${esc(x)}</span>`).join('')}${t.configs.length>3?`<span>+${t.configs.length-3}</span>`:''}</div><div class="guide-progress" aria-label="Прогресс ${progress}%"><span style="width:${progress}%"></span></div><div class="guide-actions"><span class="corner-count">${g.sectors.length} зон · ${progress}%</span><span class="link-button">Изучить →</span></div></div></button>`;}
